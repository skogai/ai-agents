...
Run this skill on a real book before declaring it done. Suggested first run: *The Mom Test*.

- [ ] Phase 1 produced answers to all five forcing questions (or halted with the pre-condition message).
- [ ] Phase 2 emitted a single decision (single | diagnose | apply) with a one-line rationale.
- [ ] Phase 3 emitted one or two SkillForge invocations in the documented format, with source attribution.
- [ ] No SKILL.md was produced by this skill; SkillForge owns that step downstream.
- [ ] When SkillForge's Phase 0 returned a >=80% duplicate match, the operator was given the three options (abandon, refine, override) before any downstream action.

If SkillForge later rejects either output at Phase 0, 2, or 4, this skill still passed: it produced a clean input. Rejection is signal, not failure.
